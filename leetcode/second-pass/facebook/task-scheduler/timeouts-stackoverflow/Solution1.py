...
class Solution:
    def leastInterval(self, tasks, n):
        ...
        def get_good_sched(task_sched, clock):
            good_sched = dict()
            earliest = maxint
            for task, sched in task_sched.items():
                if sched <= clock:
                    good_sched[task] = sched
                earliest = min(earliest, sched)
            return earliest, good_sched
        ...
        def rec(task_cnt, task_sched, clock, seq):
            ...
            min_idle = maxint
            min_sched = maxint
            earliest_sched, good_sched = get_good_sched(task_sched, clock)
            if not good_sched:
                idle = earliest_sched - clock
                min_idle = idle + rec(task_cnt, task_sched, clock + idle, seq)
            for task, sched in good_sched.items():
