...
class Solution(object):
    def search_dfs(self, x, y, word, idx, board, visited, path, rev=False):
        ...
        for (w, z) in ((x - 1, y), (x + 1, y), (x, y - 1), (x, y + 1)):
            if 0 <= w < n and 0 <= z < m:
                c = board[w][z]
                new_idx = (idx - 1) if rev else (idx + 1)
                if c == word[new_idx] and \
                        self.search_dfs(w, z, word, new_idx, board, visited, \
                                        path + [c], rev):
                    return True
        visited.remove((x, y))
        return False
    ...
    def search_word(self, word, board, char_pos):
        ...
        for (x, y), rev in (fwd_pos + rev_pos):
            ...
            c = board[x][y]
            idx = -1 if rev else 0
            found = self.search_dfs(x, y, word, idx, board, visited, [c],
                                    rev=rev)
            if found:
                return True
        return False
...
words = ["aaaa", "aaaa", "aaaa", "aaaa", "bcde", "fghi", "jklm", "nopq", "rstu",
         "vwxy", "zzzz"]
board = ["aaaaaaaaaaaaaaaa", "aaaaaaaaaaaaaaab", "aaaaaaaaaaaaaaac",
