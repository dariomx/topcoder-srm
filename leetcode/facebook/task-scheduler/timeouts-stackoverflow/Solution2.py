...
class Solution:
    def leastInterval(self, tasks, n):
        task_cnt = [0] * (ord('Z') - ord('A') + 1)
        for task, cnt in Counter(tasks).items():
            task_cnt[ord(task) - ord('A')] = cnt
        states = deque([("", task_cnt, 0)])
        min_idle = maxint
        explored = set()
        idxs = list(range(len(task_cnt)))
        while states:
            seq, task_cnt, idle = states.pop()
            print((seq, task_cnt, idle))
            if sum(task_cnt) == 0:
                min_idle = min(min_idle, idle)
            elif idle < min_idle and seq not in explored:
                last_n = set() if n==0 else set(seq[-n:])
                new_entries = False
                idxs.sort(key=lambda i: task_cnt[i])
                for i in idxs:
                    task = chr(ord('A') + i)
                    if task_cnt[i] > 0 and task not in last_n:
                        new_entries = True
                        task_cnt[i] -= 1
                        states.append((seq + task, task_cnt[:], idle))
                        task_cnt[i] += 1
                if not new_entries:
                    states.appendleft((seq + " ", task_cnt[:], idle + 1))
                explored.add(seq)
        return len(tasks) + min_idle
