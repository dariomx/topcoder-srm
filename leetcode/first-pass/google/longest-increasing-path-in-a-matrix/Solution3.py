...
class Solution(object):
    ...
    def min_gt_neighbors(self, matrix, i, j):
        gt = lambda (x, y): matrix[x][y] > matrix[i][j]
        gt_neighbors = filter(gt, self.neighbors(matrix, i, j))
        val = lambda (i, j): matrix[i][j]
        if gt_neighbors:
            min_x, min_y = min(gt_neighbors, key=val)
            is_min = lambda (x, y): matrix[x][y] == matrix[min_x][min_y]
            return ifilter(is_min, gt_neighbors)
        else:
            return []
    ...
    def search_dfs(self, matrix, start):
        ...
        while stack:
            ...
            i, j = node
            for (x, y) in self.min_gt_neighbors(matrix, i, j):
