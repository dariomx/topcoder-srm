...
def pprint2(min_x, max_x, min_y, max_y, rect):
    screen = [[0 for _ in xrange(min_x, max_x+2)] for _ in xrange(min_y, max_y+2)]
...
class Solution(object):
    def isRectangleCover(self, rect):
        ...
        :rtype: bool
        """
        min_x, max_x = maxint, 0
        min_y, max_y = maxint, 0
        for x1, y1, x2, y2 in rect:
            min_x = min(min_x, x1)
            max_x = max(max_x, x2)
            min_y = min(min_y, y1)
...
rect = [[0,0,4,1],[7,0,8,2],[5,1,6,4],[6,0,7,2],[4,0,5,1],[4,2,5,3],[2,1,4,3],[0,2,2,3],[0,1,2,2],[6,2,8,3],[5,0,6,1],[4,1,5,2]]
